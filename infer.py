...
def get_args():
    ...
    parser.add_argument('--embsize', default=128, type=int, help='Embedding dimension for discrete features')
    parser.add_argument('--hiddendim', default=256, type=int, help='Hidden dimension for model layers')
    ...
    parser.add_argument('--seq_emb_dropout_rate', default=0.2, type=float,
                        help='Dropout rate for sequence embeddings (after position encoding)')
    parser.add_argument('--attention_dropout_rate', default=0.2, type=float, help='Dropout rate for attention layers')
    parser.add_argument('--feedforward_dropout_rate', default=0.2, type=float,
...
def infer():
    ...
    def load_ids(path):
        with open(path, 'rb') as f:
            num = struct.unpack('I', f.read(4))[0]
            dim = struct.unpack('I', f.read(4))[0]
            data = np.fromfile(f, dtype=np.uint64, count=num * dim)
            return data.reshape(num, dim).flatten()
    result_path = os.environ.get('EVAL_RESULT_PATH')
    query_emb = load_emb(Path(result_path, 'query.fbin'))
    database_emb = load_emb(Path(result_path, 'embedding.fbin'))
    candidate_ids = load_ids(Path(result_path, 'id.u64bin'))
    device = args.device
    query_t = torch.from_numpy(query_emb).to(device)
    database_t = torch.from_numpy(database_emb).to(device)
    top10s_retrieved = []
    bs = args.batch_size
    for i in range(0, query_t.shape[0], bs):
        qb = query_t[i:i+bs]
        dot = qb @ database_t.T  # (bs, Nd)
        _, indices = torch.topk(dot, k=10, largest=True, dim=1)
        retrieved = candidate_ids[indices.cpu().numpy()]
        top10s_retrieved.append(retrieved)
    top10s_retrieved = np.vstack(top10s_retrieved)
    top10s_untrimmed = []
